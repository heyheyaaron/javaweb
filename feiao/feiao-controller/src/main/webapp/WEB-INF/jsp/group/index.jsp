...
<form  id="searchform">    
    ...
    <button type="button" class="btn btn-primary" id="searchbtn">查询</button>&nbsp;&nbsp; <button type="button" class="btn btn-success" id="addnew">新增用户组</button>
</form>
...
$(function(){
	//加载
	var url = "${path}/system/selectGroup.do"
	
	cmspagegroup("curpage=${curpage}",url);
	//搜索
	$("#searchbtn").click(function(){
		cmspagegroup("curpage=1&"+$("#searchform").serialize(),url);
	...
	$("#lastp").click(function(){
		...
		cmspagegroup("curpage="+cpage+"&"+$("#searchform").serialize(),url);
	})
	//点击当前页+1
	...
	$("#addnew").click(function(){
		window.location.href="${path}/system/addGroup.do?curpage="+$("#curpage").text();
	});
});

function cmspagegroup(cmsdata,url){
	...
	$.ajax({
		...
		success : function(data){
			...
			var vars = "<thead><tr><th>用户组id</th><th>用户组名名</th><th>操作</th></tr></thead>"
			
				for (var i=0;i<data.rows.length;i++){
					var vart = '<tr><td>'+data.rows[i].id+'</td><td>'+data.rows[i].name+'</td><td><a href="/system/updateGroup.do?curpage='+data.curpage+'&id='+data.rows[i].id+'">编辑</a>    <a href="javascript:void(0)" ><span id="'+data.rows[i].id+'" class="deletes">删除</span></a></td></tr>';	
					vars = vars+vart;
				}
			$("#cmstable").html(vars);
			pageful(data);
						...
						success:function(data){
							if(data.tip){
								if((Number($("#total").text())-1)%50==0){
									...
									cmspagegroup("curpage="+cpage+"&"+$("#searchform").serialize(),"${path}/system/selectGroup.do");
								}else{
