...
<form id="userForm">
<input type="hidden" name="fa_id" value="${faWare.fa_id}"/>
<table class="table table-bordered ">
    
    
    <tr>
        <td width="10%" >固件名称（型号）</td>
        <td><input type="text" name="fa_name" value="${faWare.fa_name}"/></td>
    </tr>
    <tr>
        <td >图片</td>
        <td>
        	<img src="${faWare.fa_image}" id="pic_target" width="70px" height="70px">
        	<br>
			...
			<input  type="text" id="pic_hidden" name="fa_image" style="display: none;" value="${faWare.fa_image}"/>
        </td>
    </tr>
    <tr>
        <td  >下载地址</td>
        <td><input type="text" name="fa_url" width="300px" value="${faWare.fa_url}"/></td>
    </tr>
    <tr>
        <td >排序</td>
        <td><input type="number" name="fa_sort" id="sort" value="${faWare.fa_sort}" onkeyup="this.value=this.value.replace(/[^0-9-]+/,'');"/><font color="red">    *0~999，数值最小排序越高</font></td>
    </tr>
    ...
    <tr>
        <td ></td>
        <td>
        <shiro:hasPermission name="cpware:update">
...
<script>
    $(function () {       
		$('#backid').click(function(){
				window.location.href="${path}/pro/returnWare.do?curpage=${curpage}";
		 });
		$("#savebtn").click(function(){
			...
			if(Number($("#sort").val())<0 || Number($("#sort").val())>999){
				...
				$.ajax({
					url:"${path}/pro/sureUpdateWare.do",
					type:"post",
