...
<form  id="searchform">    
   用户组名字：
    ...
    <button type="button" class="btn btn-success" id="addnew" >新增用户组</button>
    </shiro:hasPermission>
...
$(function(){
	//加载
	var url = "${path}/member/selectGroup.do"
	...
	$("#addnew").click(function(){
		window.location.href="${path}/member/addGroup.do?curpage="+$("#curpage").text();
	});
});
...
function cmspageHonor(cmsdata,url){
	...
	$.ajax({
		...
		success : function(data){
			var vars = "<thead><tr><th>用户组id</th><th>用户组名字</th><th>用户组描述</th><th>用户组图标</th><th>操作</th></tr></thead>"
			
			
			for (var i=0;i<data.rows.length;i++){
				var vart = '<tr><td>'+data.rows[i].fa_id+'</td><td>'+data.rows[i].fa_name+'</td><td>'+data.rows[i].fa_desc+'</td><td><img  src="'+data.rows[i].fa_image+'" width="40px" height="40px" ></td><td><shiro:hasPermission name="mbgroup:update"><a href="/member/updateGroup.do?curpage='+data.curpage+'&id='+data.rows[i].fa_id+'">编辑</a></shiro:hasPermission>    <shiro:hasPermission name="mbgroup:delete"><a href="javascript:void(0)" ><span id="'+data.rows[i].fa_id+'" class="deletes">删除</span></a></shiro:hasPermission>   </td></tr>';	
				vars = vars+vart;
			}
			$("#cmstable").html(vars);
			pageful(data);
						...
						success:function(data){
							if(data.tip){
								if((Number($("#total").text())-1)%50==0){
					 				...
					 				cmspageHonor("curpage="+cpage+"&"+$("#searchform").serialize(),"${path}/member/selectGroup.do");
					 			}else{
