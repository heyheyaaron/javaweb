...
jQuery.fn.Carousel = function(option){
    ...
    if(opts.setZan){
        $(".icon_zan").text(parseInt($(imgs[0]).attr("data-zan"),10));
    }
        ...
        return setInterval(function(){
            ...
            if(opts.direct===2){
                ...
                $(imgs[next]).animate({"top":0},800,function(e){
                    ...
                    if(opts.setZan){
                        $(".icon_zan").text(parseInt($(this).attr("data-zan"),10));
                    }
                });
            }else{
...
jQuery.fn.Slider = function(option){
    ...
    var animateNext = function(){
        if(el.position().left>=origLeft+totalW+(ie6?6:0)){
            el.css("left",el2.position().left+totalW+(ie6?6:0));
        }
        if(el2.position().left>=origLeft+totalW+(ie6?6:0)){
            el2.css("left",el.position().left+totalW+(ie6?6:0));
        }
        var slider1Left = el.position().left, slider2Left = el2.position().left;
        el.stop(1,0).animate({"left":slider1Left-imgW},1000,function(e){
            if(el.position().left<=origLeft-totalW+(ie6?6:0)){
                el.css("left",totalW+(ie6?-6:0));
            }
        });
        el2.stop(1,0).animate({"left":slider2Left-imgW},1000,function(e){
            if(el2.position().left<=origLeft-totalW+(ie6?6:0)){
    ...
    var animatePrev = function(){
        if(el.position().left>=origLeft+totalW*4/len+(ie6?6:0)){
            el.css("left",el2.position().left-totalW+(ie6?6:0));
        }
        if(el2.position().left>=origLeft+totalW*4/len+(ie6?6:0)){
            el2.css("left",el.position().left-totalW+(ie6?6:0));
        }
        var slider1Left = el.position().left, slider2Left = el2.position().left;
        el.stop(1,0).animate({"left":slider1Left+imgW},1000,function(e){
    ...
    timer = timerStart();
    $j(".bnr_sliderWrap").on("mouseenter",function(e){
        clearInterval(timer);
        $j(".bnr_sliderWrap").find(".tips").stop(1,0).animate({"bottom":0},400);
    }).on("mouseleave",function(e){
        $j(".bnr_sliderWrap").find(".tips").stop(1,0).animate({"bottom":-30},400);
        timer = timerStart();
    });
    $j(".bnr_sliderWrap").find(".leftbtn_picturn").on("click",function(e){
        clearInterval(timer);
        animatePrev();
    });
    $j(".bnr_sliderWrap").find(".rightbtn_picturn").on("click",function(e){
        clearInterval(timer);
        animateNext();
    })
};
...
;$j(function($){
    ...
    Index.prototype = {
        init:function(){
            ...
            $j(".notices").Carousel({"direct":2,"refresh":"#notice .icon_refresh","time":60000,"setZan":1});
            $j("#bnr_slider").Slider();
        ...
        bindEvent:function(){
            ...
            notice.on("mouseenter",function(e){
                ...
                notice.find(".icon_refresh").stop(1,0).fadeIn(200);
            }).on("mouseleave",function(e){
                notice.find(".icon_refresh").stop(1,0).fadeOut(200,function(e){
            ...
            $j(".slideUp").on("click",function(e){
               var body = $j(this).parents(".table").find(".tbody");
                ...
                if(!navigator.userAgent.match(/pad|gt-p1000/i) && $j('#footer_mobile_switch').length == 0 && navigator.userAgent.match(/android|webos|ip(hone|od)|opera (mini|mobi|tablet)|iemobile|windows.+phone|mobile|fennec|kindle (Fire)|maemo|blackberry|playbook|bb10\; (touch|kbd)|Symbian(OS)|Ubuntu Touch/i)){
                    ...
                    $j('#footer-innerLink').prepend(mobile_switch);
                }


            //客服代码点击
            $j("#service-online").on('click',function(e){
                window.open('http://live-i.meizu.com/live800/chatClient/chatbox.jsp?companyID=8957&configID=4&enterurl='+location.href+'&pagereferrer='+document.referrer+'&info=&k=1','_blank','height=768,width=1200,fullscreen=3,top=100,left=100,status=yes,toolbar=no,menubar=no,resizable=no,scrollbars=no,location=no,titlebar=no,fullscreen=no');
            });
            ...
            if($j('#header_nav').find('li a.active').length<1){
                $j.each($j('#header_nav li a'),function(i){
                    if($(this).attr('mod')=='forum_group'){
            ...
            var appqcord = $j("#appqcord"), appdown = $j("#appdown_btn").find(".appdown_btn");
            if(appdown){
                appdown.on("mouseenter",function(e){
                    appqcord.show();
                }).on("mouseleave",function(e){
                    appqcord.hide();
                });
