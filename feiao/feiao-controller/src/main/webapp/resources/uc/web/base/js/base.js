...
global.setValidate = function() {
    ...
    _addMethod("zdiyRemote", function(value, element, param) {
        ...
        var $getKey = $('#getKey');
        //因这里写死，需要判断详情页使用的getKey的ID
        if (element.id == 'ce-u-check-phone') {
            $getKey = $('#getKey-checkPhoneOld');
        } else if(element.id == 'ce-u-cTelOld') {
            $getKey = $('#getKey-activeTelOld');
        }
        ...
        var $cycode = $('#cycode');
        // hardcode...
        var isPhone = /ce-u-bind_tel|ce-u-cTelNew|phone/.test(element.id);

        if(isPhone && $cycode.length)
        	value = '00' + (($cycode.html() || $cycode.val()) - 0) + ':' + value
...
//                position = $globalName.position();
    ...
    resize: function(accountH, status) {
        ...
        return function(accountH, status) {
            ...
            if (status)  _this.status = true;
            if (winH < _this.minH) winH = _this.minH;
            if (accountH) winH = accountH + _this.fHeight;

            // _this.$footer.css('top', winH - _this.fHeight);
            if(winH > bodyH) _this.$footer.css('top', winH - _this.fHeight)
            else _this.$footer.css('top', bodyH - 1)
        };
