...
function checkFocus() {
	if(BROWSER.webkit || BROWSER.rv == "11.0"){
...
function fasteditpostvalidate(theform, noajaxpost) {
	...
	s = '';
	theform_value = getFastEditorContents();
	if(BROWSER.rv == '11.0'){
		theform_value = theform_value.replace('<p></p>','')
	}
	theform.message.value = html2bbcode(theform_value);
	...
	if(imgAttachs && imgAttachs.length > 0){
		...
		document.getElementById('fastposthiddenview').style.height = 'auto';  //for ie
	}
