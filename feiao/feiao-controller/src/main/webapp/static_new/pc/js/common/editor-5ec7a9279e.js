...
function checkFocus() {
	if(wysiwyg) {
		try {
			if(navigator.appName == 'Netscape' && !BROWSER.firefox){//兼容Netscape搜狗高速模式
...
function writeEditorContents(text) {
	if(wysiwyg) {
		if(text == '' && (BROWSER.firefox || BROWSER.opera)) {
			...
			text = '';
			if(BROWSER.rv == '11.0') {
				text = '&nbsp;';  // 临时解决方法 见禅道BUG #68  ie11下 多了一个br
			}
		}
		if(initialized && !(BROWSER.firefox && BROWSER.firefox >= '3' || BROWSER.opera)) {
			...
			text = '<!DOCTYPE html>' +
				'<html><head xmlns="http://www.w3.org/1999/xhtml" id="editorheader"><meta http-equiv="Content-Type" content="text/html; charset=' + charset + '" />' +
				...
				'<meta http-equiv="X-UA-Compatible" content="IE=EmulateIE7"></head><body contentEditable="true"  style="background-color:#FFFFFF;">' + text + '</body></html>';
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['sml', 'url', 'quote', 'code', 'free', 'hide', 'aud', 'vid', 'fls', 'attach', 'image', 'pasteword']) || typeof EXTRAFUNC['showEditorMenu'][cmd] != 'undefined' || cmd == 'tbl' || in_array(cmd, ['fontname', 'fontsize', 'forecolor', 'backcolor']) && !arg) {
		if(BROWSER.ie) {
			var tSelection = editdoc.selection;
		...
		switch(tag) {
			case 'url':
				...
				if(href != '') {
					...
					if(document.getElementsByName('parseurloff')[0] && document.getElementsByName('parseurloff')[0].checked == true){  //是否禁用url
						str = v;
					}
