...
function bbcode2html(str) {
	...
	if(!fetchCheckbox('bbcodeoff') && allowbbcode) {
		...
		str = str.replace(/\[halloween_score\](.*?)\[\/halloween_score\]/ig, '<div class="halloween-score"><span style="display:none">$1</span>万圣节狂欢趴分享内容</div>');
...
var webkit_align = false;
function dstag(options, text, tagname) {
	if(trim(text) == '') {
		return '\n';
	}
	if(!BROWSER.ie){
		text = text.replace("\n","");   // 解决粘帖时 默认<div><br></div> br不会自动去除的问题
	}
	...
	if(in_array(tagname, ['div', 'p'])) {
		...
		if(in_array(align, ['left', 'center', 'right'])) {
			...
			if(BROWSER.webkit){//chrome、safari 浏览器所见即所得模式下enter换行切换到文本模式换行消失问题
				if(!webkit_align){
					prepend = '\n' + prepend;
					webkit_align = false;
				}else {
...
function ptag(options, text, tagname) {
	...
	if(!BROWSER.ie){  //BUG #762 ie 对已发表的帖子居中问题。   options为空
		if(trim(options) == '') {
			return text + '\n';
		}
	}
...
function html2bbcode(str) {
	...
	if(!fetchCheckbox('bbcodeoff') && allowbbcode) {
		...
		str = str.replace(/<div class="halloween-score"><span style="display:none">(.*?)<\/span>万圣节狂欢趴分享内容<\/div>/ig, "[halloween_score]$1[/halloween_score]");
		...
		//str = recursion('u', str, 'simpletag', 'u');    //暂时没这个功能  且这个导致上传图片排序有问题
...
function recursion(tagname, text, dofunction, extraargs) {
	...
	do {
		...
		// var tagbegin = textlower.indexOf(open_tag);  // fix bug 807   增加此参数会导致内嵌标签（span 包含 span） 可能检测不到
		if(tagbegin == -1) {
			break;
		}
