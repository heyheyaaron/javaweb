...
$j(function(){
	...
	(function() {
        var AL = new AutoLogin({
          dname : 'bbs',
          autoLoginApi: '//bbs.meizu.cn/logging_sso.php',
          autoLogoutApi: '//bbs.meizu.cn/logout.php',
...
function replaceHost(s) {
	return s && s.replace(/"home.php/g, '"//bbs.meizu.cn/home.php')
							.replace(/"\/home.php/g, '"//bbs.meizu.cn/home.php')
							.replace(/"forum.php/g, '"//bbs.meizu.cn/forum.php')
							.replace(/"\/forum.php/g, '"//bbs.meizu.cn/forum.php');
}
//获取第一层级数据
function newsFirstLevelFun(_perPage){
	$j.ajax({
		type:"POST",
		url:"//bbs.meizu.cn/misc.php?mod=message&first=1",
		dataType:"json",
		xhrFields: {
...
function newsSecLevelFun(type,number){
	...
	$j.ajax({
		type:"POST",
		url:"//bbs.meizu.cn/misc.php?mod=message&type=" + msgType + "&perpage=" + _perPage,
		dataType:"json",
		xhrFields: {
            withCredentials: true
        },  
		success:function(data){
			...
			try{	// 有报错
				for(i=0;i<dataArray.length;i++){
					...
					contC[4] = contC[4] && contC[4].replace('"/static/image/common/systempm.png"', '"//bbs.meizu.cn/static/image/common/systempm.png"');
					contC[8] = dataArray[i].text;
					contC[8] = replaceHost(contC[8]);
					var endContC = contC.join('');
					newsCont += endContC;
				}
			}catch(e){}
			data.more_url = (data.more_url.indexOf('bbs.meizu.cn') == -1) ? ('//bbs.meizu.cn' + data.more_url) : data.more_url;
			...
			$j(".closenotice").click(function(){
				...
				$j.ajax({
					type:'POST',
					url:'//bbs.meizu.cn/misc.php?mod=message&read=1&type=' + dataType + '&id=' + newId,
					dataType:"json",
					xhrFields: {
						withCredentials: true
					},   
...
function getNoticeAll(){
	$j.ajax({
		type:"POST",
		url:"//bbs.meizu.cn/misc.php?mod=message&type=notice_all&isindex=" + $j("#mzCust").attr("rel"),
		dataType:"json",
		xhrFields: {
            withCredentials: true
        },    
