...
<form id="userForm">
<input type="hidden" name="fa_id" value="${faRank.fa_id}"/>
<table class="table table-bordered ">
    
    
    <tr>
        <td width="10%" >等级名字</td>
        <td><input type="text" name="fa_name" value="${faRank.fa_name}"/></td>
    </tr>
    <tr>
        <td >图片</td>
        <td>
        	<img src="${faRank.fa_image}" id="pic_target" width="70px" height="70px">
        	<br>
			...
			<input  type="text" id="pic_hidden" name="fa_image" style="display: none;" value="${faRank.fa_image}"/>
        </td>
    </tr>
    <tr>
        <td >等级内最低积分</td>
        <td><input type="number" name="fa_start" id="start" value="${faRank.fa_start}" onkeyup="this.value=this.value.replace(/[^0-9-]+/,'');"/><font color="red">    *范围在-99999~99999之间</font></td>
    </tr>
    <tr>
        <td >等级内最高积分${faRank.fa_notev==1}</td>
        <td><input type="number" name="fa_end" id="end" value="${faRank.fa_end}" onkeyup="this.value=this.value.replace(/[^0-9-]+/,'');"/><font color="red">    *范围在-99999~99999之间</font></td>
    </tr>
    <tr>
    	<td>发帖是否需要验证</td>
    	<td><input type="radio"  name="fa_notev" value="1" <c:if test="${faRank.fa_notev==1}"  >checked="checked"</c:if> >     是      <input type="radio" name="fa_notev"   value="0"  <c:if test="${faRank.fa_notev==0}"  >checked="checked"</c:if> >     否 </td>
    </tr>
    <tr>
    	<td>评论是否需要验证</td>
    	<td><input type="radio" name="fa_rev" value="1"  <c:if test="${faRank.fa_rev==1}"  >checked="checked"</c:if> >     是      <input type="radio" name="fa_rev"   value="0"  <c:if test="${faRank.fa_rev==0}"  >checked="checked"</c:if> >     否 </td>
    </tr>
    ...
    <tr>
        <td ></td>
        <td>
        <shiro:hasPermission name="mbrank:update">
    ...
    $(function () {       
		...
		$("#savebtn").click(function(){
			...
			if(Number($("#start").val())<-99999 || Number($("#start").val())>99999 ||Number($("#end").val())<-99999 || Number($("#end").val())>99999){
				...
				$.ajax({
					url:"${path}/member/sureUpdateRank.do",
					type:"post",
