...
<table class="table table-bordered ">
    ...
    <tr>
        <td >状态</td>
        <td>
            <input type="radio" name="isdel" value="0" <c:if test="${user.isdel==0}">checked="checked"</c:if>/> 启用
           <input type="radio" name="isdel" value="1" <c:if test="${user.isdel==1}">checked="checked"</c:if>/> 禁用
        </td>
    </tr>
    <tr>
        <td >角色</td>
    ...
    $(function () {       
		...
		$("#savebtn").click(function(){
			...
			if($("#psw1").val()!=$("#psw2").val()){
				...
				$.ajax({
					url:"${path}/system/sureUpdateUser.do",
					type:"post",
			 ...
			 success:function(data){
				 ...
				 for(var i=0;i<data.length;i++){
						 ...
						 if(istrues){
							 ...
							 var vat="<li><label class='checkbox inline'><input type='checkbox' name='groups' value='"+data[i].id+"' checked='checked' />"+data[i].name+"</label>"+"</li>";
