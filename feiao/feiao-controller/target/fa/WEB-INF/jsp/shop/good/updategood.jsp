...
<form id="userForm">
<input type="hidden" value="${faGoods.fa_id}" name="fa_id">
<table class="table table-bordered ">
    <tr>
        <td width="10%" >商品名称</td>
        <td><input type="text" name="fa_name" value="${faGoods.fa_name}"/></td>
    </tr>
    <tr>
        <td >图片</td>
        <td>
        	<img src="${faGoods.fa_image}" id="pic_target" width="70px" height="70px" >
        	<br>
			...
			<input  type="text" id="pic_hidden" name="fa_image" value="${faGoods.fa_image}" style="display: none;"/>
        </td>
    </tr>
    <tr>
        <td  >兑换价格（飞镖数量）</td>
        <td><input type="number" name="fa_cost" value="${faGoods.fa_cost}" onkeyup="this.value=this.value.replace(/[^0-9-]+/,'');"/></td>
    </tr>
    <tr>
        <td  >库存量</td>
        <td><input type="number" name="fa_num" value="${faGoods.fa_num}" onkeyup="this.value=this.value.replace(/[^0-9-]+/,'');"/></td>
    </tr>
    <tr>
        <td  >是否归还</td>
        <td><input type="radio" name="fa_isreturn" value="0" <c:if test="${faGoods.fa_isreturn==0}">checked="checked"</c:if> >否&nbsp;&nbsp;&nbsp;<input type="radio" name="fa_isreturn" value="1" <c:if test="${faGoods.fa_isreturn==1}">checked="checked"</c:if>>是</td>
    </tr>
    <tr>
    	<td>是否永久兑换</td>
    	<td><input type="radio"  name="fa_islong" value="0" <c:if test="${faGoods.fa_islong==0}">checked="checked"</c:if>>是&nbsp;&nbsp;&nbsp;<input type="radio" name="fa_islong" value="1" <c:if test="${faGoods.fa_islong==1}">checked="checked"</c:if> >否</td>
    ...
    <tr>
           <td >提示</td>
           <td>
              <textarea  cols="60" rows="10" name="fa_tips" >${faGoods.fa_tips}</textarea>
           </td>
    </tr>
    <tr>
        <td >描述</td>
        <td><textarea id="content1" name="fa_desc" cols="100" rows="8" style="width:800px;height:600px;visibility:hidden;">${faGoods.fa_desc}</textarea></td>
    </tr>
    ...
    <tr>
        <td ></td>
        <td>
        <shiro:hasPermission name="shopgood:add">
    ...
    $(function () {       
		...
		$("#savebtn").click(function(){
			document.getElementById("content1").value=KE.util.getData('content1');
				$.ajax({
					url:"${path}/shop/sureUpdateGood.do",
					type:"post",
