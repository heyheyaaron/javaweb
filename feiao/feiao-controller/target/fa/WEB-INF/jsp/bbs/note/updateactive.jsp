...
<table class="table table-bordered ">
    ...
    <tr>
        <td >报名人数上限</td>
        <td>${faNote.fa_total_people}</td>
    </tr>
    <tr>
        <td >当前报名人数</td>
        <td>${faNote.fa_finish_people}</td>
    </tr>
    <tr>
        <td >活动标题</td>
    ...
    <tr>
        ...
        <td >
			    ...
			    <shiro:hasPermission name="bbsnote:export">
			    	<button type="button" class="btn btn-warning" id="excelbtn">导出</button>&nbsp;&nbsp; 
			    	<br><font color="white">a</font>
				</shiro:hasPermission>
...
<script>
    $(function () {       
		...
		$('#excelbtn').click(function(){
				window.location.href="${path}/bbs/excelToActive.do?id=${faNote.fa_id}";
		 });
		/** 异步加载所有版块 */
		$.ajax({
		...
		//异步加载报名人信息
		$.ajax({
			url:"${path}/bbs/findAllPreMsgByNoteId.do",
			data:{"id":"${faNote.fa_id}"},
			type:"post",
			dataType:"json",
			success:function(data){
				var vars = "<thead><tr><th>会员昵称</th><th>会员手机</th><th>会员邮箱</th><th>报名时间</th><th>参与者姓名</th><th>参与者联系方式</th></tr></thead>"
				
				
				for (var i=0;i<data.length;i++){
					
					var vart = '<tr><td>'+data[i].name+'</td><td>'+data[i].tel+'</td><td>'+data[i].email+'</td><td>'+data[i].times+'</td><td>'+data[i].tn+'</td><td>'+data[i].adress+'</td></tr>';	
					vars = vars+vart;
				}
				$("#cmstable").html(vars);
			},
