...
<form id="userForm">
<input type="hidden" name="fa_id" value="${flp.fa_id}"/>
<table class="table table-bordered ">
    <tr>
        <td width="10%" >奖品名称</td>
        <td><input type="text" name="fa_name" value="${flp.fa_name}"/></td>
    </tr>
    <tr>
        <td >图片</td>
        <td>
        	<img src="${flp.fa_img}" id="pic_target" width="70px" height="70px">
        	<br>
			...
			<input  type="text" id="pic_hidden" name="fa_img" style="display: none;" value="${flp.fa_img}"/>
        </td>
    </tr>
    <tr>
           <td >奖品类型</td>
           <td>
              <input type="radio" name="fa_type" value="0"  <c:if test="${flp.fa_type==0}">checked="checked"</c:if>/>飞镖 &nbsp;&nbsp;&nbsp;<input type="radio" name="fa_type" value="1"  <c:if test="${flp.fa_type==1}">checked="checked"</c:if>/>物品
           </td>
    </tr>
    <tr>
        <td >数量</td>
        <td><input type="text" name="fa_num" value="${flp.fa_num}"  onkeyup="this.value=this.value.replace(/[^0-9-]+/,'');" /></td>
    </tr>
    <tr>
        <td >概率</td>
        <td><input type="text" name="fa_percent" value="${flp.fa_percent}"   onkeyup="this.value=this.value.replace(/[^0-9-]+/,'');" />   <font color="red"> *概率为1~999999之间的整数（所有奖品的概率之和不需要等于100，后台会根据实际数据自动生成符合的概率），单位为%</font></td>
    </tr>
    <tr>
        <td >排序</td>
        <td><input type="text" name="fa_sort" value="${flp.fa_sort}" onkeyup="this.value=this.value.replace(/[^0-9-]+/,'');" />  <font color="red"> *0~999，数值越小排序越高</font></td>
    </tr>
    ...
    <tr>
        <td ></td>
        <td>
        <shiro:hasPermission name="lotteryprize:update">
...
<script>
    $(function () {       
		$('#backid').click(function(){
				window.location.href="${path}/lottery/returnPrize.do";
		 });
		$("#savebtn").click(function(){
			
			
				$.ajax({
					url:"${path}/lottery/sureUpdatePrize.do",
					type:"post",
